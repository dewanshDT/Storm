...
class CacheDb extends _$CacheDb {
  ...
  Future<CachedNote?> note(String id) =>
      (select(cachedNotes)..where((n) => n.id.equals(id))).getSingleOrNull();
  ...
  Future<OutboxData?> outboxFor(String noteId) =>
      (select(outbox)..where((o) => o.noteId.equals(noteId))).getSingleOrNull();

  Future<List<OutboxData>> pending() =>
      (select(outbox)..orderBy([(o) => OrderingTerm.asc(o.queuedAt)])).get();
  ...
  Future<int> lastSeq() async {
    final row = await (select(
      meta,
    )..where((m) => m.key.equals(_kLastSeq))).getSingleOrNull();
