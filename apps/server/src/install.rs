...
/// The text of `data-root.conf`. Separate from the write so a test can read it.
fn drop_in_body(data_root: &Path, extra_paths: &[&Path], user: &str, group: &str) -> String {
    ...
    // systemd accepts a space-separated list on both directives below.
    ...
    let mut body = String::from("# Generated by storm-server up — do not edit by hand.\n");
    // Wait for the storage, and pull in its mounts (decision 73). Every path in
    // ReadWritePaths must exist before systemd will even spawn the process, and
    // a vault root on NFS does not exist until the mount does: prod failed 73
    // starts in a row with 226/NAMESPACE after a boot where the mount came up
    // late. A local path resolves to the root mount, so this costs nothing
    // there. It is a [Unit] key; under [Service] systemd would ignore it.
...
mod tests {
    ...
    fn the_drop_in_waits_for_every_path_it_opens() {
        // The prod layout: vaults on NFS, state and backups local.
        let body = drop_in_body(
            Path::new("/srv/storm"),
            &[
        ...
        let unit = body.find("[Unit]").expect("a [Unit] section");
        let service = body.find("[Service]").expect("a [Service] section");
        let requires = body.find("RequiresMountsFor=").expect("RequiresMountsFor");
        // Under [Service] it would be silently ignored, as StartLimitIntervalSec
        // was for months (decision 70).
        assert!(unit < requires && requires < service, "{body}");
        let line = body[requires..].lines().next().unwrap();
        assert!(line.contains("/mnt/media/Docs/storm"), "{body}");
        let rw = body[body.find("ReadWritePaths=").unwrap()..]
            .lines()
            .next()
            .unwrap();
        assert_eq!(
            line.trim_start_matches("RequiresMountsFor="),
            rw.trim_start_matches("ReadWritePaths="),
            "every path the service may write is one it waits for"
        );
    }
