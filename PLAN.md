...
**53. The development lifecycle is mandatory and codified in
`STORM_WORKFLOW.md`.** *(2026-08-16)*
An implementation slice is not complete until code verification *and* Storm
vault synchronization have both succeeded. The seven-phase lifecycle
(READ → PLAN → IMPLEMENT → VERIFY → UPDATE STORM → VERIFY STORM → REPORT)
applies to every coding agent regardless of tool. Agent-specific instruction
files (`CLAUDE.md`, `AGENTS.md`, `.cursor/rules/`) reference the canonical
workflow rather than duplicating it. This prevents the failure mode where
code is implemented and verified but the project's persistent state is left
stale, forcing the next agent to infer progress from Git history.
*Revisit if:* the workflow proves too heavy for trivial changes — in that
case, narrow the scope rather than weakening the invariant.
