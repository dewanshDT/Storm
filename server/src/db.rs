...
impl Db {
    ...
    pub fn delete_note(&mut self, id: &str, at: &str) -> Result<i64> {
        let tx = self.conn.transaction()?;
        let version: i64 = tx
            .query_row("SELECT version FROM notes WHERE id = ?1", params![id], |r| {
                r.get(0)
            })
            .optional()?
            .unwrap_or(0);
...
mod tests {
    ...
    fn keeps_version_history_for_merges() {
        ...
        assert_eq!(db.version_content("a", 1).unwrap().unwrap(), "version one\n");
        assert_eq!(db.version_content("a", 2).unwrap().unwrap(), "version two\n");
    ...
    fn full_text_search_finds_body_text() {
        let mut db = Db::open_in_memory().unwrap();
        record(&mut db, &note("a", "A.md", 1), "the quick brown fox\n", KIND_CREATED);
        record(&mut db, &note("b", "B.md", 1), "lazy dog sleeping\n", KIND_CREATED);
    ...
    fn backlinks_resolve() {
        let mut db = Db::open_in_memory().unwrap();
        record(&mut db, &note("a", "A.md", 1), "see [[Target Note]]\n", KIND_CREATED);
        record(&mut db, &note("b", "B.md", 1), "also [[Target Note]]\n", KIND_CREATED);
    ...
    fn tags_are_browsable() {
        let mut db = Db::open_in_memory().unwrap();
        record(&mut db, &note("a", "A.md", 1), "#homelab #proj\n", KIND_CREATED);
    ...
    fn delete_clears_the_index_but_keeps_history() {
        let mut db = Db::open_in_memory().unwrap();
        record(&mut db, &note("a", "A.md", 1), "#tag [[L]] words\n", KIND_CREATED);
