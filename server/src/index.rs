...
impl Indexer {
    ...
    pub fn reconcile(&mut self, dry_run: bool) -> Result<ReconcileReport> {
        ...
        for file in files {
            ...
            self.db.record_note(&row, &content, &extracted, kind, None)?;
        }
    ...
    fn row_for(
        ...
        let raw = fm.frontmatter.as_ref().map(|f| f.raw.as_str()).unwrap_or("");
        let now = now_rfc3339();
    ...
    pub fn create_note(&mut self, path: &str, content: &str) -> Result<WriteResult> {
        ...
        Ok(WriteResult { note: row, content: stamped, seq, merged: false, conflict: false })
    }
    ...
    pub fn put_note(
        ...
        let now = now_rfc3339();
        let final_content = frontmatter::set_scalars(
            outcome.text(),
            &[("id", id), ("modified", &now)],
        );
        ...
        let seq =
            self.db
                .record_note(&row, &final_content, &extracted, db::KIND_UPDATED, device_id)?;
    ...
    pub fn move_note(&mut self, id: &str, new_path: &str) -> Result<WriteResult> {
        ...
        Ok(WriteResult { note: row, content, seq, merged: false, conflict: false })
    }
    ...
    pub fn refresh_path(&mut self, rel_path: &str) -> Result<Option<i64>> {
        ...
        let now = now_rfc3339();
        let stamped =
            frontmatter::set_scalars(&content, &[("id", &id), ("modified", &now)]);
        if stamped != content {
...
mod tests {
    ...
    fn imports_an_existing_obsidian_vault() {
        ...
        assert!(frontmatter::get_scalar(
            &frontmatter::parse(&b).frontmatter.unwrap().raw,
            "id"
        )
        .is_some());
    ...
    fn fast_forward_write() {
        ...
        let result = ix
            .put_note(&created.note.id, created.note.version, "# A\n\nedited\n", None)
            .unwrap();
    ...
    fn merges_a_stale_write_to_a_different_region() {
        ...
        let their_edit = created.content.replace("Alpha.", "Alpha edited on desktop.");
        ix.put_note(&created.note.id, base_version, &their_edit, None).unwrap();

        // Our offline device writes against the now-stale base, editing the end.
        let our_edit = created.content.replace("Epsilon.", "Epsilon edited on phone.");
        let result = ix.put_note(&created.note.id, base_version, &our_edit, None).unwrap();
    ...
    fn conflicting_writes_keep_both_sides_and_stay_recoverable() {
        ...
        let result = ix.put_note(&created.note.id, base_version, &ours, None).unwrap();
    ...
    fn the_modified_timestamp_never_causes_a_conflict() {
        ...
        ix.put_note(&created.note.id, base_version, &theirs, None).unwrap();

        let ours = created.content.replace("Epsilon.", "Epsilon phone.");
        let result = ix.put_note(&created.note.id, base_version, &ours, None).unwrap();

        assert!(!result.conflict, "timestamp leaked into the merge: {}", result.content);
    ...
    fn moving_a_note_keeps_its_identity() {
        ...
        let after = ix.put_note(&id, got.version, "# A\n\nedited\n", None).unwrap();
    ...
    fn refresh_ignores_our_own_writes() {
        ...
        assert_eq!(ix.db.get_note(&created.note.id).unwrap().unwrap().version, 1);
    }
