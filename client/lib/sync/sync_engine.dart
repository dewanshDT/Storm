...
class SyncEngine extends ChangeNotifier {
  ...
  Future<SaveOutcome> save({
    ...
    try {
      ...
      if (result.conflict) {
        return SaveOutcome(SaveStatus.conflicted,
            content: result.content, version: result.meta.version);
      }
      if (result.merged) {
        return SaveOutcome(SaveStatus.merged,
            content: result.content, version: result.meta.version);
      }
  ...
  Future<SaveOutcome> move({required String id, required String newPath}) async {
    if (_online) {
      try {
      ...
      try {
        if (item.op == 'delete') {
          ...
          final moved = await api.moveNote(
              id: item.noteId, newPath: item.newPath!);
  ...
  void _openSocket() {
    ...
    final uri = Uri.parse('$url/v1/stream?token=${Uri.encodeComponent(api.token)}');
  ...
  Future<void> _store(Note note) => cache.putNote(CachedNotesCompanion.insert(
        id: note.meta.id,
        path: note.meta.path,
  ...
  Future<void> _storeWrite(String id, WriteResult result) async {
    final existing = await cache.note(id);
    await cache.putNote(CachedNotesCompanion.insert(
      id: id,
